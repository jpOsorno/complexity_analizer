...
def test_if_statement():
    """Test 5: IF-THEN-ELSE - Toma el máximo y normaliza constantes"""
    code = """
MaxOfTwo(a, b)
begin
    if (a > b) then
    ...
    print("Test 5: IF-THEN-ELSE - Max de ramas con normalización")
    print(f"  Peor caso: {result.worst_case}")
    print(f"  Mejor caso: {result.best_case}")
    print(f"  Caso promedio: {result.average_case}")
    print(f"  Explicación: {result.explanation}")
    
    # El FOR de 1 a 10 es O(10) que debe normalizarse a O(1)
    # El IF debe tomar el máximo entre O(1) y O(1) = O(1)
    assert result.worst_case == "O(1)", f"Esperado O(1), obtenido {result.worst_case}"
    assert result.best_case == "Ω(1)", f"Esperado Ω(1), obtenido {result.best_case}"
    assert result.average_case == "Θ(1)", f"Esperado Θ(1), obtenido {result.average_case}"
    print("  ✓ PASS: Constantes normalizadas a O(1)\n")
    return True


def test_constant_iterations():
    """Test 6: FOR con límites constantes - Normalizado a O(1)"""
    code = """
Fixed()
begin
    for i ← 1 to 100 do
    ...
    # 100 iteraciones = O(100) debe normalizarse a O(1)
    assert result.worst_case == "O(1)", f"Esperado O(1), obtenido {result.worst_case}"
    assert result.best_case == "Ω(1)", f"Esperado Ω(1), obtenido {result.best_case}"
    print("  ✓ PASS: O(100) normalizado a O(1)\n")
    return True
