...
if __name__ == "__main__":
    # Ejemplo: Crear AST para BubbleSort manualmente
    
    # Parámetros: A[], n
    params = [
        ArrayParamNode("A", dimensions=[None]),
        SimpleParamNode("n")
    ]
    
    # Asignación: temp ← A[j]
    temp_assignment = AssignmentNode(
        target=VariableLValueNode("temp"),
        value=ArrayAccessNode("A", [IdentifierNode("j")])
    )
    ...
    inner_for = ForNode(
        variable="j",
        start=NumberNode(1),
        end=BinaryOpNode("-", IdentifierNode("n"), IdentifierNode("i")),
        body=BlockNode(statements=[temp_assignment])
    )
    ...
    outer_for = ForNode(
        variable="i",
        start=NumberNode(1),
        end=BinaryOpNode("-", IdentifierNode("n"), NumberNode(1)),
        body=BlockNode(statements=[inner_for])
    )
    
    # Procedimiento completo
    bubble_sort = ProcedureNode(
        name="BubbleSort",
        parameters=params,
        body=BlockNode(statements=[outer_for])
    )
    
    # Programa
    program = ProgramNode(procedures=[bubble_sort])
    
    print("AST creado exitosamente:")
    print(program)
    print(f"\nProcedimiento: {program.procedures[0].name}")
    print(f"Parámetros: {[p.name for p in program.procedures[0].parameters]}")